...
if __name__ == '__main__':
    ...
    parser.add_argument("-s", "--pdb", dest="pdbid",
                        help="PDB identifier (ex: 1RX2)")
    parser.add_argument("-c", "--chainID", dest="chainID", default='A',
                        help="chain ID in the PDB for the reference sequence")
    parser.add_argument("-f", "--species", dest="species",
                        help="species of the reference sequence")
    parser.add_argument("-r", "--refseq", dest="refseq",
    ...
    parser.add_argument("-i", "--refindex", dest="i_ref", type=int,
                        help="reference sequence number in the alignment, "
                             "COUNTING FROM 0")
    parser.add_argument("-p", "--parameters", dest="parameters",
                        default=[0.2, 0.2, 0.2, 0.8], type=float, nargs=4,
                        help="list of parameters for filtering the alignment: "
                             ...
                             "details).")
    parser.add_argument("-n", "--selectSeqs", action="store_true",
                        dest="Nselect", default=False,
    ...
    parser.add_argument("-m", "--matlab", action="store_true", dest="matfile",
                        default=False,
    ...
    if options.i_ref is None:
        ...
        if options.refseq is not None and options.refpos is None:
            print("Using reference sequence but no position list provided! "
    ...
    print("Aligment size after removing sequences with non-standard amino "
    ...
    if options.i_ref is None:
        if options.pdbid is not None:
            try:
                seq_pdb, ats_pdb, dist_pdb = sca.pdbSeq(options.pdbid,
                                                        options.chainID)
                if options.species is not None:
                    try:
                        print("Finding reference sequence using species-based"
                              " best match..")
                        ...
                        print("Cant find the reference sequence using"
                              ...
                              " MSAsearch...")
                        try:
                            i_ref = sca.MSAsearch(headers_full,
                                                  sequences_full, seq_pdb)
                            options.i_ref = i_ref
                            print("reference sequence index is: %i" % (i_ref))
                            print(headers_full[i_ref])
                            print(sequences_full[i_ref])
                    ...
                    try:
                        print("Finding reference sequence using global"
                              " MSAsearch...")
                        i_ref = sca.MSAsearch(headers_full, sequences_full,
                                              seq_pdb)
                        options.i_ref = i_ref
            ...
            try:
                ...
                if options.refpos is not None:
                    ...
                    print("No reference position list provided. Using"
                          " default numbering 1 to number of positions")
    ...
    print("After filtering: alignment size is %i seqs, %i effective seqs, %i"
    ...
    if (options.Nselect):
        seqsel = sca.randSel(seqw0, int(1.5 * effseqsprelimit),
                             [seqkeep.index(i_ref)])
    ...
    if options.pdbid is not None:
        ...
        print("Size of the distance matrix: %i x %i"
