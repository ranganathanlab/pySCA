#!/usr/bin/env python
"""
The annotate_MSA script provides utilities to automatically annotate sequence
headers (for a fasta file) with taxonomic information. Currently this can be
      ...
      2) For Blast alignments, annotations can be added using the NCBI Entrez
         ...
         line argument (--giList, see below) should specify a list of gi
         numbers. These numbers are then used to query NCBI for taxonomy
         information (note that this approach requires a network connection).
